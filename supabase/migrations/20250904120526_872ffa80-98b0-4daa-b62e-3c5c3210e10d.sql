...
-- Create a function to securely insert contact submissions
...
-- Create a function to securely insert resource submissions
...
-- Remove any potential for storing contact information by adding a comment
...
-- Add additional security: Ensure resources table cannot accidentally store contact info
-- by adding a constraint that prevents common contact field names in the content
...
BEGIN
  ...
  IF NEW.content IS NOT NULL AND (
    ...
    NEW.content ~ '\w+@\w+\.\w+' -- Basic email pattern
  ) THEN
